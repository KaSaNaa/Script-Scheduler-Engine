...
void heapify_down(PriorityQueue *pq, int index) {
    ...
    while (1) {
        ...
        if (left < pq->size && pq->tasks[left].scheduled_time < pq->tasks[smallest].scheduled_time) {
            smallest = left;
        }
        if (right < pq->size && pq->tasks[right].scheduled_time < pq->tasks[smallest].scheduled_time) {
            smallest = right;
        }
        if (smallest != index) {
